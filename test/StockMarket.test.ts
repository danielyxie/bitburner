...
import {
  processOrders,
  IProcessOrderRefs,
} from "../src/StockMarket/OrderProcessing";
...
describe("Stock Market Tests", function () {
  ...
  describe("Stock Class", function () {
    describe("constructor", function () {
      ...
      it("should properly initialize props from parameters", function () {
        ...
        expect(stock.shareTxUntilMovement).toEqual(
          ctorParams.shareTxForMovement,
        );
        ...
        expect(stock.otlkMagForecast).toEqual(
          ctorParams.b ? 50 + ctorParams.otlkMag : 50 - ctorParams.otlkMag,
        );
      });
      ...
      it("should properly initialize props from range-values", function () {
        ...
        expect(stock.mv).toBeGreaterThanOrEqual(
          params.mv.min / params.mv.divisor,
        );
        expect(stock.mv).toBeLessThanOrEqual(params.mv.max / params.mv.divisor);
        expect(stock.spreadPerc).toBeGreaterThanOrEqual(
          params.spreadPerc.min / params.spreadPerc.divisor,
        );
        expect(stock.spreadPerc).toBeLessThanOrEqual(
          params.spreadPerc.max / params.spreadPerc.divisor,
        );
        expect(stock.shareTxForMovement).toBeGreaterThanOrEqual(
          params.shareTxForMovement.min,
        );
        expect(stock.shareTxForMovement).toBeLessThanOrEqual(
          params.shareTxForMovement.max,
        );
      });
    ...
    describe("#cycleForecastForecast()", function () {
      it("should increase the stock's second-order forecast by a given amount", function () {
        ...
        expect(expected).toContain(stock.otlkMagForecast);
      });
    });
    ...
    describe("#getAskPrice()", function () {
      it("should return the price increased by spread percentage", function () {
        ...
        expect(perc).toBeLessThanOrEqual(1);
  ...
  describe("Transaction Cost Calculator Functions", function () {
    describe("getBuyTransactionCost()", function () {
      ...
      it("should cap the 'shares' argument at the stock's maximum number of shares", function () {
        const maxRes = getBuyTransactionCost(
          stock,
          stock.maxShares,
          PositionTypes.Long,
        );
        const exceedRes = getBuyTransactionCost(
          stock,
          stock.maxShares * 10,
          PositionTypes.Long,
        );
    ...
    describe("getSellTransactionGain()", function () {
      ...
      it("should properly evaluate SHORT transactions", function () {
        ...
        const expected =
          shares * stock.playerAvgShortPx +
          shares * (stock.playerAvgShortPx - stock.getAskPrice()) -
          commission;
        expect(res).toEqual(expected);
      });

      it("should cap the 'shares' argument at the stock's maximum number of shares", function () {
        const maxRes = getSellTransactionGain(
          stock,
          stock.maxShares,
          PositionTypes.Long,
        );
        const exceedRes = getSellTransactionGain(
          stock,
          stock.maxShares * 10,
          PositionTypes.Long,
        );
  ...
  describe("Forecast Movement Processor Function", function () {
    ...
    function getNthForecastForecast(
      origForecastForecast: number,
      n: number,
    ): number {
      if (stock.otlkMagForecast > 50) {
        const expected =
          origForecastForecast -
          forecastChangePerPriceMovement * (n - 1) * (stock.mv / 100);
    ...
    describe("processTransactionForecastMovement() for buy transactions", function () {
      ...
      it("should properly evaluate a LONG transaction that doesn't trigger a forecast movement", function () {
        ...
        expect(stock.shareTxUntilMovement).toEqual(
          stock.shareTxForMovement - noMvmtShares,
        );
      });
      ...
      it("should properly evaluate LONG transactions that triggers forecast movements", function () {
        ...
        expect(stock.otlkMagForecast).toEqual(
          getNthForecastForecast(oldForecastForecast, 4),
        );
        expect(stock.shareTxUntilMovement).toEqual(
          stock.shareTxForMovement - noMvmtShares,
        );
      ...
      it("should properly evaluate LONG transactions of exactly 'shareTxForMovement' shares", function () {
        ...
        expect(stock.otlkMagForecast).toEqual(
          getNthForecastForecast(oldForecastForecast, 2),
        );
      ...
      it("should properly evaluate LONG transactions that total to 'shareTxForMovement' shares", function () {
        ...
        processTransactionForecastMovement(
          stock,
          Math.round(stock.shareTxForMovement / 2),
        );
        expect(stock.shareTxUntilMovement).toBeLessThan(
  ...
  describe("Transaction (Buy/Sell) Functions", function () {
    ...
    describe("buyStock()", function () {
      it("should fail for invalid arguments", function () {
        expect(buyStock({} as Stock, 1, null, suppressDialogOpt)).toEqual(
          false,
        );
      ...
      it("should not allow for transactions that exceed the maximum shares", function () {
        ...
        expect(buyStock(stock, maxShares + 1, null, suppressDialogOpt)).toEqual(
          false,
        );
      });
      ...
      it("should return true and properly update stock properties for successful transactions", function () {
        ...
        expect(Player.money.toNumber()).toEqual(0);
      });
    });

    describe("sellStock()", function () {
      it("should fail for invalid arguments", function () {
        expect(sellStock({} as Stock, 1, null, suppressDialogOpt)).toEqual(
          false,
        );
      ...
      it("should not allow for transactions that exceed the maximum shares", function () {
        const maxShares = stock.maxShares;
        expect(
          sellStock(stock, maxShares + 1, null, suppressDialogOpt),
        ).toEqual(false);
      });
      ...
      it("should return true and properly update stock properties for successful transactions", function () {
        ...
        expect(Player.money.toNumber()).toEqual(gain);
      });
      ...
      it("should cap the number of sharse sold to however many the player owns", function () {
        ...
        stock.playerAvgPx = stock.price;
        const gain = getSellTransactionGain(
          stock,
          actualShares,
          PositionTypes.Long,
        );
        ...
        expect(
          sellStock(stock, attemptedShares, null, suppressDialogOpt),
        ).toEqual(true);
        expect(stock.playerShares).toEqual(0);
        expect(stock.playerAvgPx).toEqual(0);
        expect(Player.money.toNumber()).toEqual(gain);
    ...
    describe("shortStock()", function () {
      it("should fail for invalid arguments", function () {
        expect(shortStock({} as Stock, 1, null, suppressDialogOpt)).toEqual(
          false,
        );
      ...
      it("should not allow for transactions that exceed the maximum shares", function () {
        const maxShares = stock.maxShares;
        expect(
          shortStock(stock, maxShares + 1, null, suppressDialogOpt),
        ).toEqual(false);
      });
      ...
      it("should return true and properly update stock properties for successful transactions", function () {
        ...
        expect(shortStock(stock, shares, null, suppressDialogOpt)).toEqual(
          true,
        );
    ...
    describe("sellShort()", function () {
      it("should fail for invalid arguments", function () {
        expect(sellShort({} as Stock, 1, null, suppressDialogOpt)).toEqual(
          false,
        );
      ...
      it("should not allow for transactions that exceed the maximum shares", function () {
        const maxShares = stock.maxShares;
        expect(
          sellShort(stock, maxShares + 1, null, suppressDialogOpt),
        ).toEqual(false);
      });
      ...
      it("should cap the number of sharse sold to however many the player owns", function () {
        ...
        expect(
          sellShort(stock, attemptedShares, null, suppressDialogOpt),
        ).toEqual(true);
        expect(stock.playerShortShares).toEqual(0);
        expect(stock.playerAvgShortPx).toEqual(0);
        expect(Player.money.toNumber()).toEqual(gain);
  ...
  describe("Order Placing & Processing", function () {
    ...
    describe("placeOrder()", function () {
      it("should return false when it's called with invalid arguments", function () {
        ...
        const invalid2 = placeOrder(
          stock,
          "foo" as any as number,
          2,
          OrderTypes.LimitBuy,
          PositionTypes.Long,
        );
        const invalid3 = placeOrder(
          stock,
          1,
          "foo" as any as number,
          OrderTypes.LimitBuy,
  ...
  describe("Player Influencing", function () {
    ...
    describe("influenceStockThroughServerHack()", function () {
      it("should decrease a stock's second-order forecast when all of its money is hacked", function () {
        ...
        expect(stock.otlkMagForecast).toEqual(
          oldSecondOrderForecast - forecastForecastChangeFromHack,
        );
      });
    ...
    describe("influenceStockThroughCompanyWork()", function () {
      it("should increase the server's second order forecast", function () {
        ...
        expect(stock.otlkMagForecast).toEqual(
          oldSecondOrderForecast + forecastForecastChangeFromCompanyWork,
        );
      });
      ...
      it("should be affected by performanceMult", function () {
        ...
        expect(stock.otlkMagForecast).toEqual(
          oldSecondOrderForecast + 4 * forecastForecastChangeFromCompanyWork,
        );
      });
    });
  });
