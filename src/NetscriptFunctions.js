...
import {CodingContract, ContractTypes}              from "./CodingContracts.js";
...
function NetscriptFunctions(workerScript) {
    return {
        ...
        getContractData : function (contractName, ip=workerScript.serverIp) {
            if (workerScript.checkingRam) {
                if (workerScript.loadedFns.getContractData) {
                    ...
                    workerScript.loadedFns.getContractData = true;
                    return CONSTANTS.ScriptGetContractDataRamCost;
                }
            }
            var server = getServer(ip);
            if (server == null) {
        ...
        attemptContract : function(contractName, solution, ip=workerScript.serverIp) {
            if (workerScript.checkingRam) {
                if (workerScript.loadedFns.attemptContract) {
                    return 0;
                } else {
                    workerScript.loadedFns.attemptContract = true;
                    return CONSTANTS.ScriptAttemptContractRamCost;
                }
            }
            var server = getServer(ip);
            if (server == null) {
            ...
            if(!isSolution) {
                workerScript.scriptRef.log("attemptContract() failed because the solution was invalid.");
                return false;
            }
