...
export class CrimeWork extends Work {
  ...
  finish(p: IPlayer, cancelled: boolean): string {
    ...
    const hackExp = crime.hacking_exp;
    const strExp = crime.strength_exp;
    const defExp = crime.defense_exp;
    const dexExp = crime.dexterity_exp;
    const agiExp = crime.agility_exp;
    const chaExp = crime.charisma_exp;
    const intExp = crime.intelligence_exp;
    const money = crime.money * p.crime_money_mult * BitNodeMultipliers.CrimeMoney;
