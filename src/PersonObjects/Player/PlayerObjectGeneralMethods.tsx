...
export function applyForJob(this: IPlayer, entryPosType: CompanyPosition, sing = false): IApplyForJob {
  ...
  if (!company.hasPosition(pos)) {
    const reason = `Company ${company.name} does not have position ${pos}. Player.applyToCompany() failed`;
    console.error(reason);
    return { gotJob: false, reason };
  }
  ...
  if (this.jobs[company.name] === pos.name) {
    ...
    if (!sing) {
      ...
      if (nextPos == null || !company.hasPosition(nextPos)) {
        reason = "You are already at the highest position for your field! No promotion available";
        dialogBoxCreate(reason);
      } else {
        const reqText = getJobRequirementText(company, nextPos);
        reason = "Unfortunately, you do not qualify for a promotion<br>" + reqText;
        dialogBoxCreate(reason);
      }
    }
    return { gotJob: false, reason };
