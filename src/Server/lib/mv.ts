    ...
    try{
        while (args.length > 0) {
            ...
            switch (arg) {
                ...
                case "-S":
                case "--suffix":{
                    if (args.length > 0) suffix = args.shift() as string;
                    else throw HELP_MESSAGE; 
                    break;
                }
                case "-t":
                case "--target_directory":{
                    if (args.length > 0) targetDir = args.shift() as string;
                    else throw HELP_MESSAGE; 
                    break;
                }
                case "-b":
                case "--backup":{
                    ...
                    let value:string;
                    if (args.length > 0) value = args.shift() as string;
                    else throw HELP_MESSAGE;
                    switch(value){
                        case "none":
                        ...
                        default:
                            ...
                            throw `${value} is not a valid versionning option.`;
                    }
                    break;
                }
                default:
        ...
        while(srcs.length > 0){
            ...
            if(!server.exists(src)) {
                ...
                for( let element of server.readdir(src, true)){
                ...
                server.moveFile(src, destFilename, true, verbose);
