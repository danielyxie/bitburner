...
export function HammingEncode(value: number): string { // encoding following Hammings rule
  ...
  const _sum_parity: number = HammingSumOfParity(_data.length); // get the sum of needed parity bits (for use in encoding)
  const _build = []; // init new empty array for "building"
  ...
  _build.push("x", "x", ..._data.splice(0, 1)); // init the "pre-build"

  for (let i = 2; i < _sum_parity; i++) { // add new paritybits and the corresponding data bits (pre-building array)
...
export function HammingDecode(_data: string): number { //check for altered bit and decode
  ...
  const _sum_parity = Math.ceil(Math.log2(_data.length)); // sum of parity for later use
  const count = (arr: Array<string>, val: string): number => arr.reduce((a: number, v: string) => (v === val ? a + 1 : a), 0);
  // count.... again ;)

  let _overallParity = _build.splice(0, 1).join(""); // store first index, for checking first and fix the _build properly later
  _testArray.push((_overallParity == (count(_build, "1") % 2).toString()) ? true : false); // first condition-check on the overall parity bit
  for (let i = 0; i < _sum_parity; i++) { // for the rest of the remaining parity bits we also "check"
    const _tempIndex = Math.pow(2, i) - 1 // get the parityBits Index
    const _tempStep = _tempIndex + 1 // set the stepsize
    const _tempData = [..._build] // "copy" the build-data
  ...
  let _fixIndex = 0; // init the "fixing" index amd start with 0, bc we will add the "overall"-parity later, back to its place, for "fixing"
  for (let i = 1; i < _sum_parity + 1; i++) { // simple binary adding for every boolean in the _testArray starting from 2nd index
    _fixIndex += (_testArray[i]) ? 0 : (Math.pow(2, i) / 2)
  }
  _build.unshift(_overallParity); // now we need the "overall" parity in it's place
  ...
  // if the overall is false and the sum of calculated values is greater equal 0, fix the corresponding hamming-bit
    ...
    return 0 // uhm, there's some strange going on... 2 bits are altered? How? maybe add an error-message here
  }
  // oof.. halfway through... we fixed an possible altered bit, now "extract" the parity-bits from the _build and parse the binary data
  for (let i = _sum_parity; i >= 0; i--) { // start from the last parity down the 2nd index one
